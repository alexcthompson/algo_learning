...
public:
    string longestCommonPrefix(vector<string>& strs) {
        ...
        auto first_last = minmax_element(strs.begin(), strs.end());
        string first = *first_last.first;
        string last =  *first_last.second;
        int n = min(first.length(), last.length());
        int length_prefix = 0;

        while (length_prefix < n) {
            if (first[length_prefix] == last[length_prefix]) {
                length_prefix++;
            } else {
                break;
            }
        ...
        return first.substr(0, length_prefix);
    }
};
// LC Solution class goes here
